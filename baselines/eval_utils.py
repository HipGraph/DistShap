...
def node2edge_score(edge_index: torch.Tensor, node_scores: np.array):
    """Converts node scores to edge scores: an edge score is equal to average
    of connected nodes. Needed for some baselines that only provide node scores.
...
def read_results(file_path: str) -> tuple[list, float]:
    """Reads a pickle results file and returns scores, total time and training 
    time. Note that only some methods like PGExplainer requires separate 
    training. Other methods will have 0.0 as the training time.
...
def compute_fidelity_score(
    ...
    assert testing_pred in [
        ...
        "wrong",
        "correct",
    ], "Testing prediction option is not correct!"
    ...
    for res in results:
        ...
        try:
            (
                ...
                res, data, model, sparsity, fid_type, topk, target_class, apply_abs
            )

            if (
                testing_pred == "wrong" and correct_class == init_pred_class
            ):  # skip correct preds
                continue
            if (
                testing_pred == "correct" and correct_class != init_pred_class
            ):  # skip wrong preds
                continue
...
def fidelity_table(
    ...
    for i, dataset_name in enumerate(
        tqdm(dataset_names, disable=tqdm_hide, leave=False)
    ):
        model, data, config = get_model_data_config(
            dataset_name, load_pretrained=True, device=device, full_data=full_data
        )
