...
from dataset.expl_indices import get_expl_indices


def get_model(model_name, config, num_features, num_classes,
    ...
    Args:
        ...
        log_softmax_return (bool, optional): whether to return raw output or
            log softmax. Defaults to False.
    ...
    if model_name == 'GCNModel':
        ...
        model = GATModel(hidden_channels=hidden_channels,
                         num_features=num_features,
                         num_classes=num_classes,
...
def get_model_data_config(dataset_name, device='cpu', load_pretrained=True,
                          ...
                          explain_subset: str = None):
    """Gets model, data and configuration for a given dataset name. Check the
        dataset.configs.py for the configuration details.
    Args:
        dataset_name (str): name of the dataset
        device (str, optional): 'cpu' or 'cuda'. Defaults to 'cpu'.
        load_pretrained (bool, optional): whether to load pretrained model.
            Defaults to True.
        log_softmax_return (bool, optional): whether to return raw output or
            log softmax. Defaults to False.
        full_data (bool, optional): full data or explanation data. If True for
            large datasets, only loads the subset of the data used in
            explanation that created by neighbor sampling. It doesn't make any 
            difference for others. Defaults to False.
        explain_subset (str, optional): subset of the test nodes to explain.
            Defaults to None. test_nodes are already defined in the config.
            This is extra option for large datasets like Reddit.
    ...
    if 'Cora' in dataset_name:
        ...
        dataset = CitationFull(root=f'{root_path}/data/CitationFull',
                               name='DBLP',
                               transform=NormalizeFeatures())
    ...
    elif 'ogbn-products' in dataset_name:
        ...
        dataset = PygNodePropPredDataset(name='ogbn-arxiv',
                                         root=f'{root_path}/data/ogbn-arxiv',
    ...
    model = get_model(model_name, config, dataset.num_features,
                       dataset.num_classes,
                       log_softmax_return=log_softmax_return).to(device)
    
    if load_pretrained:
        try:
            state_dict = torch.load(f"{root_path}/pretrained/{dataset_name}"
                                    f"_pretrained.pt", weights_only=True,
                                    map_location=device)
            model.load_state_dict(state_dict)
            model.eval()
        except FileNotFoundError:
            print(f"No pretrained model found for {dataset_name}. "
                  "Don't forget to train the model.")
    ...
    if 'ogbn' in dataset_name:
        ...
        tmp_name = dataset_name.split('_')[0]
        split = get_split(f'{root_path}/pretrained/', tmp_name, data)
        (data.train_mask, data.val_mask,
         data.test_mask) = split[0], split[1], split[2]

    if dataset_name in ['Reddit', 'ogbn-products'] and not full_data:
        data = torch.load(f"{root_path}/pretrained/{dataset_name}_"
                          "explain_data.pt")
    ...
    config['test_nodes'] = data.test_mask.nonzero().cpu().numpy()[:,0]
    # only use the first 100 test nodes for GNN explanation.
    # User has option to not use this.
    config['test_nodes'] = config['test_nodes'].tolist()[:100]

    if explain_subset:
        dataset_name = dataset_name.split('_')[0] # remove the custom model name
        config['test_nodes'] = get_expl_indices(dataset_name, explain_subset)

    return model, data, config


def generate_balanced_split(data, num_train_per_class, num_val_per_class):
    """Generates a balanced train, validation, test split for a given dataset.
    This is used for datasets that do not have built-in splits.
    ...
    for c in range(num_classes):
        ...
        tr_idx = idx[np.random.choice(len(idx), size=num_train_per_class,
                                      replace=False)]
        train_mask[tr_idx] = True

        val_idx = np.array([i for i in idx if i not in tr_idx])
        val_idx = val_idx[np.random.choice(len(val_idx), size=num_val_per_class,
                                           replace=False)]
...
def get_split(pretrained_dir, dataset_name, data):
    """Gets the train, validation, test split for a given dataset. If the split
    is not found, creates a new split and saves it to the disk. This is used
    for datasets that do not have built-in splits.
    ...
    if os.path.exists(f_name):
        split = torch.load(f_name, weights_only=False)
    else:
        split = generate_balanced_split(data, num_train_per_class=30,
