...
class GATModel(torch.nn.Module):
    def __init__(self, hidden_channels,
        ...
        self.convs = nn.ModuleList(
            [GATConv(num_features, hidden_channels,
            ...
            [GATConv(hidden_channels * heads, num_classes,
                      add_self_loops=add_self_loops, dropout=dropout, heads=1)])
        self.softmax_fn = nn.LogSoftmax(dim=-1) if log_softmax_return \
            else nn.Identity()
    ...
    def forward(self, x, edge_index, edge_weight=None):
        ...
        x = self.softmax_fn(x) # applied based on parameter.
        return x
