...
class GNNShapExplanation:
    ...
    def __init__(self, node_idx: int, nplayers: int, base_value: int,
                 ...
                 sub_edge_index: Tensor, sub_nodes: Tensor,
                 sub_node_labels: Tensor,
    ...
    def get_edge_names(self) -> list:
        return self.__get_edge_names()
    ...
    def plot_graph(self, topk: int = 25, save_path: str = None, pos=None, show_scores: bool = False,
        ...
        color_list = ['orange', 'blue', 'red', 'cyan',
                    '#D3B98C', "lightblue", '#D3A38C', 'red', 'yellow',
                    'pink', 'grey', 'purple', 'gold', 'green', 'brown', 'black']
        ...
        node_colors = [color_list[self.sub_node_labels
                                  [np.where(self.sub_nodes.cpu().numpy() == n
                                            )[0][0]]] for n in nodes]
    ...
    def result2dict(self) -> dict:
        ...
        return {'node_id': self.node_idx, 'scores': self.shap_values,
                ...
                'time_comp_graph': self.time_comp_graph,
                'time_sampling': self.time_sampling,
                'time_predictions': self.time_predictions,
                'time_solver': self.time_solver,
                'computed_samples': self.computed_samples,
                'mask_time': self.kwargs.get('mask_time', None),
                'solver_stats': self.kwargs.get('solver_stats', None),
                'pred_stats': self.kwargs.get('pred_stats', None),
                }
